...
namespace {
  ...
  const char* labels[label_count] = {"down", "up", "left", "right"};
  ...
  if (done_just_triggered) {
    ...
    for (int y = 0; y < raster_height; ++y) {
      ...
      for (int x = 0; x < raster_width; ++x) {
        const uint8_t* pixel = &raster_buffer[(y * raster_width * raster_channels) + (x * raster_channels)];
    ...
    for (int i = 0; i < raster_byte_count; i++) {
      model_input->data.uint8[i] = (raster_buffer[i] + model_input->params.zero_point);
    }
    ...
    count++;
    avgInferenceTime = (ElapsedTime + avgInferenceTime * (count - 1)) / count; 
    Serial.println(avgInferenceTime);
